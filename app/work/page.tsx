    ...
    <main className="min-h-screen bg-[#030712] overflow-x-hidden w-full">
      ...
      <section className="pt-32 pb-24 relative">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6">
          ...
          {/* Tabs */}
          <div className="flex flex-wrap gap-2 justify-center mb-16 p-1 bg-gray-900/30 border border-gray-700/30 rounded-full w-fit mx-auto">
            {tabs.map((tab, index) => (
              <button
                key={tab.value}
                onClick={() => setActiveTab(index)}
                className={`relative px-6 py-2.5 rounded-full text-sm font-medium transition-colors duration-300 cursor-pointer z-10 ${
                  activeTab === index
                    ? "text-blue-200"
